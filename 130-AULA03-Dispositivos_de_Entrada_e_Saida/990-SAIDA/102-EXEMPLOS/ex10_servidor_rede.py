...
DEF_CLIENTTARGETPORT = 9192;

DEF_CLIENTRECVADDRESS = '0.0.0.0';
DEF_CLIENTRECVPORT = 9191;
...
class ServidorRede(th.Thread):
    ...
    def processMessage(self, message):
        ...
        arr = dataStr.split(DEF_SEP);
        
        cmd = arr[0];
        if cmd == DEF_REQ_ACTION_GET:
            objtype = arr[1];
            if objtype == DEF_REQ_OBJTYPE_QUESTION:
                pos = int(arr[2]);

                result = DEF_LISTA_PERGUNTAS[pos];
                resultXml = "<?xml version='1.0'?><Pergunta posicao=" + str(pos) + "><CDATA[[" + result + "]]></Pergunta>";
                ...
                self.sendMessage(resultXml);

        elif cmd == DEF_REQ_ACTION_LISTALL:
            objtype = arr[1];
            if objtype == DEF_REQ_OBJTYPE_QUESTION:
                resultXml = "<?xml version='1.0'?><Perguntas>";
                sz = len(DEF_LISTA_PERGUNTAS);
                for i in range(0, sz):
                    result = DEF_LISTA_PERGUNTAS[i];
                    resultXml += resultXml + "<Pergunta posicao=" + str(i) + "><CDATA[[" + result + "]]></Pergunta>";
                resultXml = resultXml + "</Perguntas>";
...
def procExecuta():
    servidor = ServidorRede();
    servidor.startMessageLoop();
